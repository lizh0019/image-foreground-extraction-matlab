function SegmentExcGC(InImgFile, OutImgFile, OutSegResultFilename, topX, topY, bottomX, bottomY)
% SEGMENTGC  - Main GrabCut segmentation function
% SEGMENTGC(IMNAME, IH, ALG) - ImName - Image to segment
% IH - Image Handle
%tic

ImName = strcat('D:\MMRS\Upload\'  , InImgFile);         %Input MAT file
OutImName = strcat('D:\MMRS\Upload\'  , OutImgFile); %Output Index text file
OutSegResult = strcat('D:\MMRS\Upload\'  , OutSegResultFilename);         %Input MAT file
%OutSegItr = strcat('D:\MMRS\Upload\'  , OutSegItrFilename); %Output Index text file
...
TMP=num2str(bottomX);
CbottomX=str2num(TMP);

TMP=num2str(bottomY);
CbottomY=str2num(TMP);

fp=[CtopY, CtopX; CbottomY, CbottomX];
%fp = ginput(2);
%fp = round(fp);
...
% post-processing
[connectedLabel,num] = bwlabel(SegImage,8);
areaL = zeros(1,num);
for i = 1:num
    [r,c] = find(connectedLabel == i);
    areaL(i) = length(r);
end
[C,Ind] = max(areaL);
SegImageI=SegImage;

SegImage(connectedLabel ~= Ind) = 0;
SegImageI(connectedLabel ~= Ind) = 1;
SegImageI(connectedLabel == Ind) = 0;

SegImage = imfill(SegImage);
%SegImageI = imfill(SegImageI);
...
SegImageI = repmat(SegImageI,[1,1,3]);
NewBackGnd = uint8(SegImageI) .* uint8(I);

H = fspecial('disk',7);
blurred = imfilter(NewBackGnd,H,'replicate');

SegNewImage = SegNewImage + blurred;

% SegImage1(SegImage1 == 0) = 255;
% SegImage1(SegImage1 == 1) = 0;
% SegImage1 = repmat(SegImage1,[1,1,3]);
% SegNewImage = SegNewImage + uint8(SegImage1);

%SegNewImage=segmentResultShow(SegMask,I);
SegNewImage=segmentResultShow(SegMask,SegNewImage);

imwrite(SegNewImage,OutImName,'jpg');
save(OutSegResult, 'SegMask');
...
% save(OutSegResult, 'SegMask');
% save OutSegResult SegMask SegResult;

% Required for AutoCutRefine
%save(OutSegItr,'L', 'MeanColors', 'FLabels', 'BLabels', 'FCClusters',...
%    'FCovs', 'FWeights', 'BCClusters', 'BCovs', 'BWeights');
%save OutSegItr L MeanColors FLabels BLabels FCClusters FCovs FWeights BCClusters BCovs BWeights;
