...
%CF change this FY = fgpixels(:,1);
%CF change this FX = fgpixels(:,2);

%CF change this BY = bgpixels(:,1);
%CF change this BX = bgpixels(:,2);

% LZ
%CF change this FindicesStr = sub2ind(size(I1),FX,FY);
%CF change this BindicesStr = sub2ind(size(I1),BX,BY);
%CF change this size(FindicesStr)
%CF change this size(BindicesStr)
%CF change this 'indices'

%CF change this Findices = union(Findices,FindicesStr);
%CF change this [WrongB WrIndB] = intersect(Bindices,FindicesStr);
%CF change this size(WrIndB)
%CF change this 'Wrindb'
%CF change this Bindices(WrIndB) = [];


%CF change this Bindices = union(Bindices,BindicesStr);
%CF change this [WrongF WrIndF] = intersect(Findices,BindicesStr);
%CF change this size(WrIndF)
%CF change this 'Wrinf'
%CF change this Findices(WrIndF) = [];


%CF change this L = watershed(I(:,:,1));        %%%% Doing watershed on the red channel -- SEE if you can do it on the color image
